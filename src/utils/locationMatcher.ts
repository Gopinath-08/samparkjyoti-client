...
/**
 * Filter products by location proximity
 */
export function filterProductsByLocation(products: any[], userLocation: string): any[] {
  ...
  const normalizedUserLocation = normalizeLocation(userLocation);
  if (process.env.NODE_ENV === 'development') {
    console.log('🔍 Filtering products by location:', {
      userLocation,
      normalizedUserLocation,
  ...
  const filteredProducts = products.map(product => {
    ...
    // Calculate location match score
    const locationMatch = locationsMatch(userLocation, productLocation);
    const locationScore = locationMatch ? 100 : 0;

    if (process.env.NODE_ENV === 'development') {
      console.log(`📍 Product "${product.name}" (${product.id}):`, {
        productLocation,
        normalizedProductLocation,
        locationMatch,
        locationScore
      });
    }
    ...
    return {
      ...
      locationMatch,
      locationScore,
      normalizedLocation: normalizedProductLocation
    };
  }).sort((a, b) => {
    // Sort by location match first, then by other criteria
    if (a.locationMatch && !b.locationMatch) return -1;
  ...
  const matchingProducts = filteredProducts.filter(product => product.locationMatch);
  if (process.env.NODE_ENV === 'development') {
    console.log('✅ Product location filtering complete:', {
      totalProducts: filteredProducts.length,
      matchingProducts: matchingProducts.length,
      matchingProductNames: matchingProducts.map(product => product.name)
    });
  }
 ...
 */
export function getLocationBasedProductRecommendations(products: any[], userLocation: string, limit: number = 10): any[] {
  const locationFilteredProducts = filterProductsByLocation(products, userLocation);

  // Return top matching products
  return locationFilteredProducts.slice(0, limit);
}
